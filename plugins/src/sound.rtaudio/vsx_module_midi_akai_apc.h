...
public:
    ...
    void module_info(vsx_module_info* info)
    {
      ...
      info->identifier = "sound;midi;akai_apc_40_controller";
      info->out_param_spec =
        ...
        for (int track_sel_i = 0; track_sel_i < 8; track_sel_i++)
        {
          for (int column_i = 0; column_i < 8; column_i++)
          {
            for (int row_i = 0; row_i < 5; row_i++)
            {
              ...
              //sprintf(name,"multi_clip_launch_%d_%d_%d", track_sel_i + 1, column_i + 1, row_i + 1);
    ...
    void handle_messages()
    {
        ...
        //vsx_printf("midi message size %d\n", mess.size() );
        if(mess.size() > 0)
        {
